...
All paths ABSOLUTE — run INSIDE chroot only.
"""
import os
...
# umask=0077 MUST be a quoted string — Python 3 rejects leading zeros as integers
...
print("[9/12] displaymanager.conf ...")
...
print("[11/12] polkit rule — Calamares runs without password prompt ...")
...
print("[12/12] Calamares autostart — 15s delay, gnome-terminal launch ...")
# KEY FIX: use gnome-terminal so window is always visible
# sleep 15 — waits for GNOME Shell + polkit agent to be fully ready
# pkexec removed — polkit rule above allows passwordless exec for ridos user
